...
#define LOWER_PER_THRESHOLD 660
#define UPPER_PER_THRESHOLD 720
...
static uint8_t ActiveLED = LED_OFF;
...
void Beacon_Receiver_ISR(void)
{
	...
	uint32_t Period = (Time-LastTime)/40;
	//printf("%i.",(Time-LastTime)/40);
	//if the frequency of detection matches the expected beacon frequency
	if ((Period>=LOWER_PER_THRESHOLD)&&(Period<=UPPER_PER_THRESHOLD)){
		counter++;
	}
	else counter /= 2;
	printf("%i.",counter);
...
static void Init_LED(void)
{
	...
	//set up periodic mode
	HWREG(WTIMER4_BASE+TIMER_O_TAMR)=(HWREG(WTIMER4_BASE+TIMER_O_TAMR)&~TIMER_TAMR_TAMR_M)|TIMER_TAMR_TAMR_PERIOD;
